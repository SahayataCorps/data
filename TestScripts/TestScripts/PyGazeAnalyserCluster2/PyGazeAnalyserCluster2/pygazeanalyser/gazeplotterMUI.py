# -*- coding: utf-8 -*-
"""
This is a test file altered to work on Multi-user image graphs
"""
# native
import os
# external
...
def draw_heatmap(fixations, dispsize, imagefile=None, durationweight=True, alpha=0.5, savefilename=None):
	...
	# FIXATIONS
	fix = parse_raw_fixations(fixations)
...
def multiuser_image_scanpath_heatmap(raw_fixation, fixations, saccades, dispsize, imagefile=None, alpha=0.5, savefilename=None):
	fig, ax = draw_heatmap(raw_fixation, dispsize, imagefile, durationweight=True, alpha=0.5, savefilename=None)
...
def draw_scanpath(fixations, saccades, dispsize, imagefile=None, alpha=0.5, savefilename=None):
	...
	fix = parse_fixations(fixations)
	# draw fixations
	ax.scatter(fix['x'],fix['y'], s=(1 * fix['dur']*30 ), c=COLS['chameleon'][2], marker='o', cmap='jet', alpha=alpha, edgecolors='none')
...
def parse_fixations(fixations):
	...
	for fixnr in range(len( fixations)):
		ex, ey, cost, dur, nop  = fixations[fixnr]
		fix['x'][fixnr] = ex
		fix['y'][fixnr] = ey
		fix['dur'][fixnr] = dur
	
	return fix
def parse_raw_fixations(fixations):
	...
	# get all fixation coordinates
	for fixnr in range(len( fixations)):
		cost, start, end, dur, ex, ey  = fixations[fixnr]
		fix['x'][fixnr] = ex
		fix['y'][fixnr] = ey
		fix['dur'][fixnr] = dur
