"""
This is a test file to create single user fixation and scanpath
"""
...
# data = pd.read_csv("fixation_data.csv")
# fixData = list(data.iloc[:,:].values)
scanData1 = pd.read_csv("1200CSV/0_scanpath_data.csv")
sas1 = list(scanData1.iloc[:,:].values)
sasFix1 = list(scanData1.iloc[:,0:5].values)
draw_scanpath(sasFix1, sas1, "test.mp4")

# capt = cv2.VideoCapture("test.mp4")
# out = cv2.VideoWriter("out.avi",cv2.VideoWriter_fourcc(*'XVID'),24,(1280,720))
# i = 0
# x = 0
# durr=0
# while True:
